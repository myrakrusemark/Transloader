...
function db_upload
{
    ...
    time=$(utime)
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES $CURL_PARAMETERS -i -o "$RESPONSE_FILE" --upload-file "$FILE_SRC" "$API_UPLOAD_URL/$ACCESS_LEVEL/$FILE_DST?oauth_consumer_key=$APP_KEY&oauth_token=$USER_KEY&oauth_signature_method=PLAINTEXT&oauth_signature=$APP_SECRET%26$USER_SECRET&oauth_timestamp=$time&oauth_nonce=$RANDOM"
...
function db_ckupload
{
    ...
    while ([ $OFFSET -ne $FILE_SIZE ]); do      
        ...
        #Uploading the chunk...
        time=$(utime)
        $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --upload-file "$CHUNK_FILE" "$API_CHUNKED_UPLOAD_URL?$CHUNK_PARAMS&oauth_consumer_key=$APP_KEY&oauth_token=$USER_KEY&oauth_signature_method=PLAINTEXT&oauth_signature=$APP_SECRET%26$USER_SECRET&oauth_timestamp=$time&oauth_nonce=$RANDOM"
    ...
    while (true); do
        ...
        time=$(utime)
        $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --data "upload_id=$UPLOAD_ID&oauth_consumer_key=$APP_KEY&oauth_token=$USER_KEY&oauth_signature_method=PLAINTEXT&oauth_signature=$APP_SECRET%26$USER_SECRET&oauth_timestamp=$time&oauth_nonce=$RANDOM" "$API_CHUNKED_UPLOAD_COMMIT_URL/$ACCESS_LEVEL/$FILE_DST"
...
{
    time=$(utime)
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --data "oauth_consumer_key=$APP_KEY&oauth_token=$USER_KEY&oauth_signature_method=PLAINTEXT&oauth_signature=$APP_SECRET%26$USER_SECRET&oauth_timestamp=$time&oauth_nonce=$RANDOM" "$API_INFO_URL"
...
function db_download
{
    ...
    time=$(utime)
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES $CURL_PARAMETERS --globoff -D "$RESPONSE_FILE" -o "$FILE_DST" "$API_DOWNLOAD_URL/$ACCESS_LEVEL/$FILE_SRC?oauth_consumer_key=$APP_KEY&oauth_token=$USER_KEY&oauth_signature_method=PLAINTEXT&oauth_signature=$APP_SECRET%26$USER_SECRET&oauth_timestamp=$time&oauth_nonce=$RANDOM"
...
function db_delete
{
    ...
    print " > Deleting \"$1\"... "  
    time=$(utime)
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --data "oauth_consumer_key=$APP_KEY&oauth_token=$USER_KEY&oauth_signature_method=PLAINTEXT&oauth_signature=$APP_SECRET%26$USER_SECRET&oauth_timestamp=$time&oauth_nonce=$RANDOM&root=$ACCESS_LEVEL&path=$FILE_DST" "$API_DELETE_URL"
...
function db_list
{
    ...
    print " > Listing \"$1\"... "  
    time=$(utime)
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" "$API_METADATA_URL/$ACCESS_LEVEL/$DIR_DST?oauth_consumer_key=$APP_KEY&oauth_token=$USER_KEY&oauth_signature_method=PLAINTEXT&oauth_signature=$APP_SECRET%26$USER_SECRET&oauth_timestamp=$time&oauth_nonce=$RANDOM"
...
if [ "$APP_KEY" ]; then
    ...
    OAUTH_ACCESS_TOKEN="$USER_KEY"
    
    #Checking the loaded data
    if [ -z "$APP_KEY" -o -z "$APP_SECRET" -o -z "$USER_SECRET" -o -z "$USER_KEY" ]; then
...
#NEW SETUP...
    ...
    while (true); do
        ...
        echo -ne "\n > App key is $APP_KEY, App secret is $APP_SECRET and Access level is $ACCESS_MSG, it's ok? [y/n]"
        read answer
        if [ "$answer" == "y" ]; then
            break;
        fi
    ...
    echo -ne "\n > Token request... "
    time=$(utime)
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error -i -o $RESPONSE_FILE --data "oauth_consumer_key=$APP_KEY&oauth_signature_method=PLAINTEXT&oauth_signature=$APP_SECRET%26&oauth_timestamp=$time&oauth_nonce=$RANDOM" "$API_REQUEST_TOKEN_URL"
    ...
    while (true); do
        ...
        time=$(utime)
        $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error -i -o $RESPONSE_FILE --data "oauth_consumer_key=$APP_KEY&oauth_token=$OAUTH_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$APP_SECRET%26$OAUTH_TOKEN_SECRET&oauth_timestamp=$time&oauth_nonce=$RANDOM" "$API_ACCESS_TOKEN_URL"
        ...
        if [ -n "$USER_KEY" -a -n "$USER_SECRET" -a -n "$OAUTH_ACCESS_UID" ]; then
            echo -ne "OK\n"
            
            #Saving data
            echo "APPKEY:$APP_KEY" > "$APP_KAY"
            echo "APPSECRET:$APP_SECRET" >> "$APP_SECRET"
            echo "ACCESS_LEVEL:$ACCESS_LEVEL" >> "$ACCESS_LEVEL"
            echo "OAUTH_ACCESS_TOKEN:$USER_KEY" >> "$USER_KEY"
            echo "OAUTH_ACCESS_TOKEN_SECRET:$USER_SECRET" >> "$USER_SECRET"
